...
class LinearModel:
    ...
    def activation(self, x):
        """
        Implement a sigmoid activation function that accepts a float and returns
        ...
        if type(x) is bool or type(x) is list or type(x) is np.ndarray:
            raise ValueError('boolean, list or numpy array is passed in')

        return 1/(1+np.exp(-x))

    def forward(self, inputs):
        """
        Implement the forward pass (inference) of a the model.
        ...
        z = np.dot(weights[0:2], inputs) + bias
